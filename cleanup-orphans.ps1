...
$targets = @(
    ...
    # NOTE: /api/tutorial is NOT here - it serves active home/compare/invest tutorials
    # via app/components/Tutorial.tsx. Keep that endpoint intact.
    ...
    @{ Path = 'app\api\training\route.ts';                Type = 'file'; Reason = 'Training API for removed standalone curriculum feature' },
    @{ Path = 'app\api\training';                         Type = 'dir';  Reason = 'Empty after route.ts deletion' },
...
$refsToCheck = @(
    ...
    @{ Pattern = '/api/training'; Description = 'references to training API' }
)
...
foreach ($r in $refsToCheck) {
    $matches = Get-ChildItem -Path 'app' -Recurse -Include '*.tsx','*.ts' -ErrorAction SilentlyContinue |
        ...
        Where-Object { $_.Name -ne 'alpaca-screener.ts' } |
        Where-Object { $_.FullName -notmatch '\\data\\screener\.ts$' } |
        Select-String -Pattern ([regex]::Escape($r.Pattern)) -SimpleMatch -List |
        Where-Object {
            # Drop matches that are Alpaca's external API URL (contains "screener" but not the local route)
            $_.Line -notmatch 'data\.alpaca\.markets/v2/screener'
        }
    if ($matches) {
        $foundRefs = $true
