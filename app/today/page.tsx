...
import {
  LogOut, TrendingUp, TrendingDown, RefreshCw, ArrowLeft, Eye, Zap, Globe, AlertTriangle, BarChart3, Sun, Moon, Radio, Check, Calendar, Clock, ShieldCheck, Target
} from 'lucide-react'
...
interface NewsMover {
  ...
  timeframe: string
  relatedNews: string[]
  // Session 2+5: confidence score and grounded verification
  confidence?: number
  verified?: boolean
...
interface NewsPageData {
  ...
  cachedAt?: string
  ageMinutes?: number
  // Session 2+5: regime context and news source counts
  regime?: {
    ...
    <div className="rounded-xl border transition-all duration-200"
      ...
      <div className="p-4 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Confidence badge (Session 2+5) */}
            {typeof mover.confidence === 'number' && (
              <div
                className="flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-mono font-semibold"
                style={{
                  background: mover.confidence >= 80 ? 'rgba(52,211,153,0.12)'
                    : mover.confidence >= 70 ? 'rgba(251,191,36,0.12)'
                    : 'rgba(148,163,184,0.10)',
                  color: mover.confidence >= 80 ? '#34d399'
                    : mover.confidence >= 70 ? '#fbbf24'
                    : '#94a3b8',
                  border: `1px solid ${mover.confidence >= 80 ? 'rgba(52,211,153,0.25)' : mover.confidence >= 70 ? 'rgba(251,191,36,0.25)' : 'rgba(148,163,184,0.18)'}`,
                }}
                title={`Model confidence: ${mover.confidence}%`}
              >
                {mover.confidence}%
              </div>
            )}
            {/* Verification badge (Session 2+5) */}
            {mover.verified === true && (
              <div
                ...
                title={mover.verificationNote || 'Verified against Reuters/Bloomberg/WSJ'}
              >
                <ShieldCheck size={9} />
                <span>verified</span>
              </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3 border-t" style={{ borderColor: `${accentColor}15` }}>
          ...
          {/* Verification sources (Session 5) */}
          {mover.verified && mover.verificationSources && mover.verificationSources.length > 0 && (
            <div className="rounded-lg p-3" style={{ background: 'rgba(96,165,250,0.05)', border: '1px solid rgba(96,165,250,0.18)' }}>
              ...
              {mover.verificationNote && (
                <p className="text-[11px] text-white/60 mb-1.5 italic">{mover.verificationNote}</p>
              )}
              <div className="space-y-1">
                {mover.verificationSources.slice(0, 3).map((url, i) => {
                  let domain = url
      ...
      {data && !loading && (
        <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {/* Regime pill (Session 5) */}
                    {data.regime && (
                      <span
                        className="text-[10px] font-mono px-2 py-0.5 rounded-full"
                        style={{
                          background: data.regime.label === 'risk-on' ? 'rgba(52,211,153,0.12)'
                            : data.regime.label === 'risk-off' ? 'rgba(248,113,113,0.12)'
                            : 'rgba(148,163,184,0.10)',
                          color: data.regime.label === 'risk-on' ? '#34d399'
                            : data.regime.label === 'risk-off' ? '#f87171'
                            : '#94a3b8',
                          border: `1px solid ${data.regime.label === 'risk-on' ? 'rgba(52,211,153,0.25)' : data.regime.label === 'risk-off' ? 'rgba(248,113,113,0.25)' : 'rgba(148,163,184,0.18)'}`,
                        }}
                        title={data.regime.context}
                      >
                        {data.regime.label}
                        {data.regime.vixLevel !== null && ` · VIX ${data.regime.vixLevel.toFixed(1)}`}
                      </span>
                    )}
                  </div>
          ...
          <div className="max-w-4xl mx-auto px-3 sm:px-5 py-4 sm:py-6 space-y-6 sm:space-y-8">
            ...
            {/* Hit rate widget (Session 3+5) — shows actual accuracy over time */}
