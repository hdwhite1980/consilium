...
export default function SettingsPage() {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
      <div className="px-5 py-3 border-b" style={{ background: 'var(--surface2)', borderColor: 'var(--border)' }}>
        <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--text3)' }}>{title}</span>
      </div>
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      <header className="flex items-center gap-3 px-4 py-3 border-b sticky top-0 z-10"
        ...
        <button onClick={() => router.push('/')} className="flex items-center gap-1.5 text-xs hover:opacity-70" style={{ color: 'var(--text3)' }}>
          <ArrowLeft size={13} /> Back
        </button>
        <div className="w-px h-4" style={{ background: 'var(--border)' }} />
        <span className="text-sm font-bold" style={{ color: 'var(--text)' }}>Settings</span>
      ...
      <div className="max-w-xl mx-auto px-4 py-6 space-y-5">
        ...
        <Section title="Account">
          <div>
            <div className="text-[10px] font-mono uppercase tracking-widest mb-1" style={{ color: 'var(--text3)' }}>Email</div>
            <div className="text-sm font-mono" style={{ color: 'var(--text)' }}>{user?.email}</div>
          </div>
        </Section>
        ...
        <Section title="Appearance">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Text size</div>
            </div>
              ...
              {([
                ...
                return (
                  <button
                    ...
                    style={{
                      background: active ? 'rgba(167,139,250,0.15)' : 'var(--surface2)',
                      border: `1px solid ${active ? '#a78bfa' : 'var(--border)'}`,
                      color: active ? '#a78bfa' : 'var(--text)',
                      outlineColor: '#a78bfa',
                    }}>
                    <div className="text-sm font-semibold">{opt.label}</div>
                    <div className="text-[10px] font-mono mt-0.5" style={{ color: active ? '#a78bfa' : 'var(--text3)' }}>{opt.sample}</div>
                  </button>
                )
              })}
            </div>
          ...
          <div className="pt-3 border-t" style={{ borderColor: 'var(--border)' }}>
            <div className="flex items-center gap-2 mb-2">
              <Palette size={14} style={{ color: 'var(--text2)' }} />
              <div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Theme</div>
            </div>
        ...
        <Section title="Subscription">
            ...
            <div className="space-y-2 pt-2 border-t" style={{ borderColor: 'var(--border)' }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => upgradePlan('standard')}
                  ...
                  style={{ background: 'var(--surface2)', border: isStandard ? `1px solid #fbbf24` : '1px solid var(--border)' }}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <Zap size={12} style={{ color: '#fbbf24' }} />
                    <span className="text-xs font-bold" style={{ color: 'var(--text)' }}>Standard</span>
                    {isStandard && <CheckCircle size={11} style={{ color: '#fbbf24' }} />}
                  </div>
                  <div className="text-lg font-bold font-mono" style={{ color: 'var(--text)' }}>$29</div>
                  <div className="text-[10px]" style={{ color: 'var(--text3)' }}>/month</div>
                </button>
                <button onClick={() => upgradePlan('pro')}
                  className="flex flex-col items-start p-3 rounded-xl text-left hover:opacity-80 transition-all"
                  style={{ background: isPro ? 'rgba(167,139,250,0.08)' : 'var(--surface2)', border: isPro ? '1px solid rgba(167,139,250,0.4)' : '1px solid var(--border)' }}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <Crown size={12} style={{ color: '#a78bfa' }} />
                    <span className="text-xs font-bold" style={{ color: 'var(--text)' }}>Pro</span>
                    {isPro && <CheckCircle size={11} style={{ color: '#a78bfa' }} />}
                  </div>
                  <div className="text-lg font-bold font-mono" style={{ color: 'var(--text)' }}>$49</div>
                  <div className="text-[10px]" style={{ color: 'var(--text3)' }}>/month</div>
                </button>
              </div>
            </div>
          ...
          {isStandard && (
            <div className="text-xs space-y-1 pt-2 border-t" style={{ borderColor: 'var(--border)', color: 'var(--text3)' }}>
              <div className="font-semibold mb-1.5" style={{ color: 'var(--text2)' }}>Pro adds:</div>
          ...
          {activeFactor ? (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>2FA enabled</div>
                  <div className="text-xs" style={{ color: 'var(--text3)' }}>Authenticator app</div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="text-[10px] font-mono uppercase tracking-widest mb-1.5" style={{ color: 'var(--text3)' }}>Verification code</div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setEnrolling(false); setQrCode(''); setVerifyCode('') }}
                  className="px-4 py-2 rounded-lg text-xs hover:opacity-70" style={{ color: 'var(--text3)' }}>
                  Cancel
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ShieldOff size={16} style={{ color: 'var(--text3)' }} />
                <div>
                  <div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>2FA disabled</div>
                  <div className="text-xs" style={{ color: 'var(--text3)' }}>Add an extra layer of security</div>
                </div>
              </div>
        ...
        <Section title="Change Password">
          ...
          <div>
            <div className="text-[10px] font-mono uppercase tracking-widest mb-1.5" style={{ color: 'var(--text3)' }}>New password</div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs" style={{ color: 'var(--text3)' }}>Sent to {user?.email}</div>
              </div>
              <button onClick={() => setNotifPrefs(p => ({ ...p, email_enabled: !p.email_enabled }))}
                className="w-10 h-5 rounded-full transition-all relative"
                style={{ background: notifPrefs.email_enabled ? '#a78bfa' : 'var(--surface2)', border: `1px solid ${notifPrefs.email_enabled ? '#a78bfa' : 'var(--border)'}` }}>
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>SMS alerts</div>
                <div className="text-xs" style={{ color: 'var(--text3)' }}>Text message to your phone</div>
              </div>
              <button onClick={() => setNotifPrefs(p => ({ ...p, sms_enabled: !p.sms_enabled }))}
                className="w-10 h-5 rounded-full transition-all relative"
                style={{ background: notifPrefs.sms_enabled ? '#a78bfa' : 'var(--surface2)', border: `1px solid ${notifPrefs.sms_enabled ? '#a78bfa' : 'var(--border)'}` }}>
                <div className="w-3.5 h-3.5 rounded-full bg-white absolute top-0.5 transition-all"
            ...
            {notifPrefs.sms_enabled && (
              <div>
                <div className="text-[10px] font-mono uppercase tracking-widest mb-1.5" style={{ color: 'var(--text3)' }}>Phone number</div>
                <input value={notifPrefs.phone} onChange={e => setNotifPrefs(p => ({ ...p, phone: e.target.value }))}
                  ...
                  style={{ background: 'var(--surface2)', borderColor: 'var(--border)', color: 'var(--text)' }} />
                <div className="text-[10px] mt-1" style={{ color: 'var(--text3)' }}>Include country code e.g. +1 for US</div>
              </div>
            )}
            <div>
              <div className="text-[10px] font-mono uppercase tracking-widest mb-1.5" style={{ color: 'var(--text3)' }}>Minimum severity to notify</div>
              <div className="flex gap-2">
                {(['watch', 'alert', 'urgent'] as const).map(s => (
                  <button key={s} onClick={() => setNotifPrefs(p => ({ ...p, min_severity: s }))}
                    className="flex-1 py-1.5 rounded-lg text-xs font-semibold capitalize transition-all"
                    style={{
                      background: notifPrefs.min_severity === s ? 'rgba(167,139,250,0.12)' : 'var(--surface2)',
                      color: notifPrefs.min_severity === s ? '#a78bfa' : 'var(--text3)',
                      border: `1px solid ${notifPrefs.min_severity === s ? 'rgba(167,139,250,0.3)' : 'var(--border)'}`,
                    }}>
                    {s}
                  </button>
                ))}
        ...
        <Section title="Sign Out">
          <div className="flex items-center justify-between">
            <p className="text-xs" style={{ color: 'var(--text3)' }}>Sign out of all devices</p>
        ...
        <p className="text-center text-[11px] pb-4" style={{ color: 'var(--text3)' }}>
          Wali-OS · <a href="mailto:support@wali-os.com" style={{ color: 'var(--text3)' }}>support@wali-os.com</a>
        </p>
      </div>
    </div>
  )
