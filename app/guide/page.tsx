...
const SECTIONS: GuideSection[] = [
  {
    ...
    content: (
      <>
        <P>When you enter a ticker, Wali-OS runs a multi-stage AI analysis that streams in real time. Each stage builds on the last — you watch the reasoning develop before the final verdict arrives.</P>
        <H>What happens during an analysis</H>
        <P>The system starts by pulling live data: current headlines, price action, all technical indicators, fundamentals, options flow, smart money positioning, and regulatory/legislative context. This data is assembled into a complete signal bundle before any analysis begins.</P>
        <P>Multiple analytical perspectives then work through the data, each with a specific role. Arguments are made, challenged, and refined using live data fetched mid-analysis. The process is designed so no single perspective dominates — conclusions must survive scrutiny before reaching the verdict.</P>
        <H>The Verdict</H>
        <P>The final verdict is delivered by a senior analytical role that has read the complete reasoning chain. It produces: a directional signal (BULLISH/BEARISH/NEUTRAL), confidence score, ATR-derived entry, stop loss, and take profit levels, time horizon, three probability-weighted scenarios, and a plain English action plan.</P>
        <Tip>The concessions section is the most important part of the analysis. When an analytical role concedes a key point, it signals genuine weakness in the thesis. A verdict with major concessions deserves smaller position sizing regardless of the confidence number.</Tip>
        <H>The Invalidation Trigger</H>
        <P>Every verdict includes a single condition that would make the entire thesis wrong. If that condition is met — set a price alert for it — re-run the analysis immediately. The outcome will likely be very different.</P>
        <Warn>Wali-OS analysis is for informational purposes only. It is not financial advice. Never risk money you cannot afford to lose.</Warn>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        <P>The sidebar shows 24+ live indicators computed from market data before the debate starts. These feed directly into the analysis.</P>
        ...
        <P><strong className="text-white">ATR(14)</strong> — Average True Range in dollars and %. Wali-OS uses 2× ATR for stop placement and 3× ATR for targets. Stops tighter than 1× ATR will be hit by normal daily noise.</P>
        ...
        <P><strong className="text-white">Relative Strength vs Sector</strong> — Stock performance minus sector ETF performance. A stock up 2% when its sector is up 9% is underperforming by 7% — hidden weakness that can undermine a bullish thesis.</P>
        ...
        <P><strong className="text-white">Earnings Implied Move</strong> — ATM straddle cost vs historical actual move. When options are overpriced vs history, this is flagged and the options strategy recommendation adjusts accordingly.</P>
        <H>Conviction Score</H>
        <P>Aggregates all signals into a -100 to +100 score. Adjusted for market regime — HIGH FEAR discounts bullish signals. Influences the confidence ceiling on the final verdict.</P>
  ...
  },
  {
    ...
    content: (
      <>
        <P>Before running an analysis, select which analytical lens is applied. Same data, different interpretation.</P>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <H>Log a Trade</H>
        <P>After any BULLISH or NEUTRAL verdict, click "💰 Log trade" to record the entry price, share count, and signal. This links to the Reinvestment Tracker where you can track P&L and get AI-powered reinvestment ideas.</P>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        ...
        <H>🔥 Invest</H>
        <P>A separate journey-based experience for building a portfolio from any starting amount. Enter how much you have to invest ($5, $50, $500 — anything), and Wali-OS finds stage-appropriate stocks sized to your exact balance.</P>
        ...
        <P>Live sector performance is pulled from the Macro dashboard to find 5 picks from today's strongest sectors — all within your stage's price range. Each pick includes a specific catalyst, entry zone, ATR-adjusted stop, target, and suggested share count for your available capital.</P>
        ...
        <P>The Invest page is built for any starting balance — $5 or $5,000. Enter how much you have to invest and Wali-OS finds stocks priced and sized for that exact amount. Six fire milestones track your progress: Spark ($0–$10) → Ember ($10–$50) → Flame ($50–$200) → Blaze ($200–$1K) → Inferno ($1K–$10K) → Free ($10K+).</P>
        <P>Each stage changes the stock price range based on your deployable capital — targeting 10–30 shares per position so every holding feels meaningful. At $5 you get $1–3 stocks. At $500 you get $20–40 stocks. At $2,000 you get $50–100 stocks. Wali-OS recalibrates automatically when you cross a milestone.</P>
        ...
        <P>Log trades after council analyses to track live P&L. When you close a trade, the realized gain becomes "available cash." Wali-OS then generates three tiered reinvestment strategies — Aggressive (50% of gains into a high-conviction idea), Moderate (25-40% into a strategic play), and Conservative (10-20% into a lower-risk option) — each with a specific entry, stop, and target.</P>
        ...
        <P>Runs the full analysis on two stocks simultaneously, then produces a definitive head-to-head verdict. Shows side-by-side conviction scores, risk/reward bars, strengths and weaknesses for each, and a clear "if you can only pick one" recommendation.</P>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        <P>The Options Strategy section appears below the verdict after each analysis. It shows a derivatives recommendation based on the verdict, conviction score, IV conditions, and GEX signal.</P>
        <H>Strategy Selection Logic</H>
        <P>BULLISH high conviction → long calls or bull call spreads. BULLISH moderate → covered calls or bull call spread. BEARISH high conviction → long puts or bear put spreads. NEUTRAL → both sides are shown for reference.</P>
        <H>Earnings Implied Move</H>
        <P>When earnings are within 30 days, Wali-OS calculates the ATM straddle cost vs the stock's historical average actual move over the last 4 earnings. If options are priced at 2× the historical move, it flags "OPTIONS OVERPRICED — selling premium is favored." This is a specific, actionable edge that most retail tools don't provide.</P>
        <H>GEX and Options Strategy</H>
        <P>Strong negative GEX means defined-risk strategies (spreads) are recommended over naked directional bets — because negative GEX amplifies moves in both directions and creates whipsaw risk for options buyers.</P>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <H>Short Interest</H>
        <P>Percentage of float sold short and days to cover. Above 25% = squeeze candidate — good news can trigger rapid covering. Above 15% = heavily shorted. The put/call ratio is used as a proxy when direct short interest data is unavailable.</P>
      </>
    ),
  },
]
...
const FAQ_GENERAL: FAQItem[] = [
  { q: 'What is the Invest page?', a: "The Invest page is a journey-based tracker for growing any starting balance. You enter how much you have to invest, and Wali-OS finds stage-appropriate stocks sized to your exact capital. Six fire milestones track your progress from Spark ($0–$10) to Free ($10K+). At each stage, the stock price range adjusts automatically so every position feels like a real holding — not a lottery ticket. It uses live sector data from the Macro dashboard to find 5 picks from today's strongest sectors." },
  ...
  { q: 'What is Wali-OS?', a: "Wali-OS is an AI-powered stock and crypto analysis platform that uses a proprietary multi-stage analytical process before giving you a verdict. You get a specific signal (BULLISH/BEARISH/NEUTRAL), entry price, ATR-derived stop loss, take profit levels, time horizon, and a full plain English explanation. It covers US stocks, major crypto, and your own portfolio." },
  { q: 'How is this different from other AI stock tools?', a: "Most AI analysis tools give you one model's opinion. Wali-OS runs a proprietary multi-perspective process where each analytical stage challenges the previous one using live data. You see every argument and concession — not just a final answer. The verdict must survive internal scrutiny before it reaches you." },
  ...
  { q: 'Does it support crypto?', a: "Yes — BTC, ETH, SOL, BNB, XRP, ADA, AVAX, DOGE, and 15+ more. Crypto uses CoinGecko for real-time prices and OHLCV data. All 24+ technical indicators work identically. Fundamentals are replaced with on-chain metadata (market cap, 24h/7d change, ATH, circulating supply). The full analysis process runs on crypto exactly as it does for equities." },
  { q: 'What does the free trial include?', a: "The 7-day free trial includes full access to every feature — main analysis, crypto, Macro dashboard, Portfolio, Reinvestment Tracker, Compare and all indicator data. No credit card required to start." },
]

const FAQ_SIGNALS: FAQItem[] = [
  { q: 'What is the conviction score?', a: "The conviction score aggregates all signals across technicals, fundamentals, smart money, options flow, and macro context into a net score from -100 to +100. It's adjusted for market regime — HIGH FEAR discounts bullish signals. The conviction score influences the confidence ceiling on the final verdict — a weakly positive score can't produce a strongly confident BULLISH verdict." },
  { q: 'Why does the confidence score matter for position sizing?', a: "The confidence score is a measure of signal agreement, not a probability of profit. 85%+ confidence with no significant concessions = full intended position. 70-84% = 75%. 55-69% = 50%. Below 55% = 25% or wait. Any verdict with 2+ significant concessions should drop one tier regardless of the confidence number." },
  { q: 'What are ATR-derived stops and why does the council use them?', a: "ATR (Average True Range) measures how much the stock moves on a typical day. A stop tighter than 1× ATR will be triggered by normal daily volatility before the trade has a chance to work. Wali-OS uses 2× ATR as the minimum stop placement and 3× ATR as the baseline target, giving a 1.5:1 risk/reward starting point. You can see the specific dollar levels in the sidebar under ATR(14)." },
...
const FAQ_TECHNICAL: FAQItem[] = [
  ...
  { q: 'What is Ichimoku Cloud and why does it matter?', a: "Ichimoku Kinko Hyo is one of the most comprehensive single indicators in technical analysis — it shows trend direction, momentum, and support/resistance simultaneously. Price above the cloud = structurally bullish (26+ sessions of price structure support this). A TK cross (Tenkan crossing Kijun while above the cloud) is an institutional entry signal. Cloud position is one of the first factors evaluated when it's decisive." },
  { q: 'How does relative strength vs sector work?', a: "Wali-OS computes your stock's period return minus the sector ETF's period return over the same period. Stock up 2%, sector up 9% = -7% relative strength. This exposes hidden weakness — the stock is rising with the tide, not leading it. Lagging relative strength is a key challenge to any bullish thesis when a stock is underperforming its peers." },
...
const FAQ_OPTIONS: FAQItem[] = [
  { q: 'What is the Options Strategy section?', a: "After each analysis, the Options Strategy section shows a specific derivatives recommendation based on the verdict, conviction score, IV conditions, and GEX signal. For BULLISH verdicts it typically recommends long calls or bull call spreads depending on IV levels. For BEARISH it recommends puts or put spreads. When IV is elevated (options expensive), it may suggest selling premium instead." },
  { q: 'What is the earnings implied move?', a: "When earnings are within 30 days, Wali-OS calculates the cost of the at-the-money straddle and compares it to the stock's historical average actual move over the last 4 earnings. If options are priced at 2× the historical move, the analysis notes 'OPTIONS OVERPRICED — vol selling is favored.' This is an edge most retail platforms don't provide." },
