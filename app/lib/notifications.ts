...
// Uses raw fetch against Resend + Twilio REST APIs directly
// No npm packages needed — zero build-time dependencies
// ─────────────────────────────────────────────────────────────
...
export async function sendAlertSMS(n: AlertNotification) {
  ...
  try {
    ...
    const params = new URLSearchParams({ Body: body, From: process.env.TWILIO_PHONE_NUMBER, To: n.phone })
